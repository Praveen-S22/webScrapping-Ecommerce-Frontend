...
function App() {
  ...
  async function fetchProducts() {
    let productData = await axios.get(
      "https://webscrappingecommerce.herokuapp.com/"
    );
  ...
  async function fetchOneProduct(query) {
    ...
    let temp = productData.data.filter((res) =>
      res.title.toLowerCase().includes(query)
    );
    setProducts(temp);
  }
  async function addProduct(query) {
    let productData = await axios.post(
      "https://webscrappingecommerce.herokuapp.com/",
      { query }
    );
    if (productData.data.response === "present") fetchOneProduct(query);
    else {
      fetchProducts();
    }
  }
